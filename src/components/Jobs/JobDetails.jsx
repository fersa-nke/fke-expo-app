...
import { setSelectedJobId } from "../../redux/Jobs/JobsActions";
import {
  getReports,
  removeFromReports,
  setSelectedReportId,
...
} from "../../redux/ReduxConsants";
import {synsAllData} from '../../redux/Reports/ReportsAction'
    ...
    : <>
        ...
        <ListItem label="Bearing Model" value={job[JOBKEYMapper.BEARINGMODEL] ?  job[JOBKEYMapper.BEARINGMODEL][0]?.Name : ''} />
          ...
          {job[JOBKEYMapper.DEDATAMATRIX] ? (
            <ListItem
              label="New DE DataMatrix"
              value={job[JOBKEYMapper.DEDATAMATRIX]}
            />
          ) : (
            <ListItem
              label="New DE Batch"
              value={job[JOBKEYMapper.DEBATCHNUMBER]}
            />
          )}
          {job[JOBKEYMapper.NDEDATAMATRIX] ? (
            <ListItem
              label="New NDE DataMatrix"
              value={job[JOBKEYMapper.NDEDATAMATRIX]}
            />
          ) : (
            <ListItem
              label="New NDE Batch"
              value={job[JOBKEYMapper.NDEBATCHNUMBER]}
            />
          )}
          {job[JOBKEYMapper.SENSORBATCHNUMBER] ? (
            <ListItem
              label="Sensor Batch"
              value={job[JOBKEYMapper.SENSORBATCHNUMBER]}
            />
          ) : (
            <ListItem
              label="Sensor DataMatrix"
              value={job[JOBKEYMapper.SENSORDATAMATRIX]}
            />
          )}
          <ListItem
            label="Replacement Date"
            value={job[JOBKEYMapper.FAILUREDATE]}
          />
          <ListItem
            label="Exchange Type"
            value={
              job[JOBKEYMapper.EXCHANGETYPE]
                ? job[JOBKEYMapper.EXCHANGETYPE][0]?.Name
                : ""
            }
          />
          <ListItem
            ...
            value={
              job[JOBKEYMapper.REASONS]
                ? job[JOBKEYMapper.REASONS][0]?.Name
                : ""
            }
          />
          <ListItem
            label="Wind Farm"
            value={
              job[JOBKEYMapper.CUSTOMERWINDFARM]
          ...
          <ListItem
            ...
            value={
              job[JOBKEYMapper.WINDLOCATION]
                ? job[JOBKEYMapper.WINDLOCATION][0]?.Name
                : ""
            }
          />
          <ListItem
            label="Wind Turbine"
            value={job[JOBKEYMapper.WINDTURBINE]}
          />
          <ListItem
            label="State"
            value={
              job[JOBKEYMapper.STATE] ? job[JOBKEYMapper.STATE][0]?.Name : ""
            }
          />
          <ListItem
            label="Generator Model"
            value={
              job[JOBKEYMapper.GENERATORMODEL]
                ? job[JOBKEYMapper.GENERATORMODEL][0]?.Name
                : ""
            }
          />
          <ListItem
            label="Bearing Model"
            value={
              job[JOBKEYMapper.POSITION]
                ? job[JOBKEYMapper.POSITION][0]?.Name
                : ""
            }
          />
          {job[JOBKEYMapper.REMOVEDDEBATCHNUMBER] ? (
            <ListItem
              label="Removed Batch"
              value={job[JOBKEYMapper.REMOVEDDEBATCHNUMBER]}
            />
          ) : (
            <ListItem
              label="Removed DE DataMatrix"
              value={job[JOBKEYMapper.REMOVEDDEDATAMATRIX]}
            />
          )}
          {job[JOBKEYMapper.REMOVEDNDEBATCHNUMBER] ? (
            <ListItem
              label="Removed NDE Batch"
              value={job[JOBKEYMapper.REMOVEDNDEBATCHNUMBER]}
            />
          ) : (
            <ListItem
              label="Removed NDE DataMatrix"
              value={job[JOBKEYMapper.REMOVEDNDEDATAMATRIX]}
            />
          )}
          <ListItem
            label="Removed Bearing Brand"
            value={
              job[JOBKEYMapper.REMOVEDBEARINGBRAND]
                ? job[JOBKEYMapper.REMOVEDBEARINGBRAND][0]?.Name
                : ""
            }
          />
          <ListItem
            label="Removed Bearing Type"
            value={
              job[JOBKEYMapper.REMOVEDBEARINGTYPE]
                ? job[JOBKEYMapper.REMOVEDBEARINGTYPE][0]?.Name
                : ""
            }
          />
          <ListItem
            ...
            value={
              job[JOBKEYMapper.NEWBEARINGBRAND]
                ? job[JOBKEYMapper.NEWBEARINGBRAND][0]?.Name
                : ""
            }
          />
          <ListItem
            ...
            value={
              job[JOBKEYMapper.NEWBEARINGTYPE]
                ? job[JOBKEYMapper.NEWBEARINGTYPE][0]?.Name
                : ""
            }
          />
...
const ReportsView = () => {
  const showSyncReports = useSelector(state => state.reportsReducer.showSyncReports)
  const navigation = useNavigation();
  ...
  const navigateAddReport = () => {
    const reportName = `${selectedJobTitle?.split("-JB")[0]}/${
      reports.length + 1
    }`;
  ...
  return (
    <ScrollView>
      ...
      <View style={{alignItems: "flex-end", paddingBottom: 10}} >
         <TouchableOpacity onPress={() =>dispatch(synsAllData(selectedJobId))}>
           <Image
        ...
        <Report
          list={reports}
          onHandlePress={navigateToReportDetails}
          ReportEdit={onEditClick}
          ReportDelete={handleRemoveReport}
        />
      </View>
      <Ripple style={GBStyles.addReportBtn} onPress={navigateAddReport}>
        <Row>
...
const JobDetails = ({ route }) => {
 ...
 // console.log(viewType, "viewTypeviewTypeviewTypeviewTypeviewTypeviewTypeviewType")
    ...
    <>
      ...
      <Row style={[Styles.card]}>
        ...
        <View style={{ flex: 1, marginRight: 8 }}>
          <Image
            source={
              job[JOBKEYMapper.SENSORDATAMATRIX] ||
              job[JOBKEYMapper.NDEDATAMATRIX] ||
              job[JOBKEYMapper.DEDATAMATRIX]
                ? BarCode
                : defaultIcon

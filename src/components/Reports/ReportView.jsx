...
function ReportView({ route }) {
  ...
  const attachments = useSelector(
    (state) => state.attachmentsReducer.attachments
  );
  const downloadedFile = useSelector(
    (state) => state.attachmentsReducer.downloadedFile
  );
  ...
  const downLoadFile = async () => {
    ...
    try {
      const filename =
        FileSystem.documentDirectory + openFileObj.fileName + "." + extenstion;
  ...
  const save = async (filename, mimetype, remoteURI) => {
    if (Platform.OS === "android") {
      ...
      if (permissions.granted) {
        const base64 = await FileSystem.writeAsStringAsync(
          filename,
          downloadedFile,
          {
            encoding: FileSystem.EncodingType.Base64,
        ...
        await FileSystem.StorageAccessFramework.createFileAsync(
          ...
          .then(async (uri) => {
            await FileSystem.writeAsStringAsync(uri, downloadedFile, {
              encoding: FileSystem.EncodingType.Base64,
            });
            alert(
              "Downloaded succesfully! Check in" + permissions.directoryUri
            );
          })
      ...
      <ScrollView>
        <View style={GBStyles.container}>
          <Row style={{ marginVertical: 24 }}>
            ...
            <Ripple
              style={[Styles.reportUpload, { marginLeft: 6 }]}
              onPress={showActionSheet}
            >
              <Row sty>
                <IconComp name="File" size={16} color={theme.textBlue} />
        ...
        <Ripple style={GBStyles.upload} onPress={pickImage}>
          ...
          <Button
            ...
            style={{ marginTop: 20 }}
            type="Secondary"
            onPress={() => navigation.navigate("JobDetails", { id: 1 })}
          />
        </View>
      </ScrollView>
                ...
                <>
                  {IconsType[openFileObj.fileType] !== "Pdf" ? (
                    <Image
                      ...
                      style={Styles.image}
                      source={{
                        uri: `data:image/png;base64,${downloadedFile}`,
                      }}
                    />
                  ) : (
                    <View

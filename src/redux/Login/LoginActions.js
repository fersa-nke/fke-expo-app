...
const inserLoggedInUser = (response) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      tx.executeSql(
        "INSERT OR IGNORE INTO customers (Email, Password,Role) VALUES (?, ?, ?)",
        [response.Email, response.Password, response.Role],
        (_, results) => {
          resolve(
  ...
  return async function loginThunk(dispatch, getState) {
    ...
    NetInfo.fetch().then((state) => {
      if (state.isConnected) {
        ...
        console.log("***************************offline connected");
        getLoggedInUser()
          .then((res) => {
            console.log(res[0], "res-test");
            if (res !== undefined) {
              console.log("able to fetch the offline user...",res);
            // dispatch({ type: ADD_SIGNED_USER_DATA, payload: true });

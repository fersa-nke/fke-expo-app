...
export const getJobsBySearchQuery = (query, formObj) => {
  return async (dispatch, getState) => {
    ...
    if (formObj.windFarm) {
      squery =
        squery + "(" + JobMapper.WINDFARM + ",eq," + formObj.windFarm + ")";
    }
    if (formObj.windLocation) {
      ...
      squery =
        squery +
        "(" +
        JobMapper.WINDLOCATION +
        ",eq," +
        formObj.windLocation +
        ")";
    }
      ...
      if (state.isConnected) {
        API.POST(`${BASE_URL}`, token, formData)
          ...
          .catch((error) => {
            ...
            displayToast("error", JSON.stringify(error));
            //Hide Loader
          }); // JSON data parsed by `data.json()` call
      } else {
        dispatch({

...
import {
  ...
  GET_EXCHANGE_TYPES,
  GET_MODELS,
  GET_REASON_OF_CHANGES,
  GET_SHAFT_POSITIONS,
  GET_WIND_FARMS,
  GET_WIND_LOCATIONS,
  GET_STATES,
  GET_LUBRICATION_GRADES,
  GET_LUBRICATION_TYPES,
  LOADING_DATA,
} from "../ReduxConsants";
// Define action types
  ...
  return async (dispatch, getState) => {
    ...
    await API.GET(`nocodb/data/FG-MRO-Tracker/${URL}`, token)
      .then((res) => {
        ...
        try {
          if (res) {
            ...
            console.log("Unable to fetch test ashish singh");
          }
        } catch (e) {
      ...
      .catch((error) => {
        ...
        console.log("error auth error from axios -------------->", error);
...
export function callAPI(URL, dispatchType, token = "") {
  return async (dispatch, getState) => {
    ...
    API.GET(`nocodb/data/FG-MRO-Tracker/${URL}`, token)
      .then((res) => {
        ...
        try {
          if (res) {
            ...
            console.log("Unable to fetch ashish kumar");
          }
        } catch (e) {
      ...
      .catch((error) => {
        console.log("error auth error testing -------------->", error);
        //dispatch(offlineLogin(true));
        //Hide Loader
      }); // JSON data parsed by `data.json()` call
  };
...
function updateAPIMapper(list) {
  list.forEach((element) => {
    if (element.Name) APIConfig[element.Name] = element["API Name"];
  });
...
function updateKEYMapper(list) {
  list.forEach((element) => {
    if (element.Name) KEYMapper[element.Name] = element["TAG Name"];
  });
